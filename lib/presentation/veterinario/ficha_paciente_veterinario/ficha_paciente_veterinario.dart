...
class VetPatientProfileScreen extends StatefulWidget {
  static const String name = 'VetPatientProfileScreen';
  const VetPatientProfileScreen({super.key});

  @override
  State<VetPatientProfileScreen> createState() => _VetPatientProfileScreenState();
}

class _VetPatientProfileScreenState extends State<VetPatientProfileScreen> {
  @override
  void initState() {
    super.initState();
    ...
    return Scaffold(
      ...
      body: Consumer<MascotaProvider>(
        builder: (context, provider, child) {
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  Text('Error al cargar información del paciente', style: Theme.of(context).textTheme.titleLarge),
          ...
          if (detalle == null) {
            return const Center(child: Text('No hay información del paciente'));
          }
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _PatientInfoSection(detalle: detalle),
                  ...
                  _InfoCard(
                    ...
                    child: _OwnerInfo(detalle: detalle),
                  ),
                  ...
                  _InfoCard(
                    ...
                    child: detalle.vacunas.isEmpty
                        ? _EmptyState(message: 'No tiene vacunas registradas')
                        : _VaccineList(vacunas: detalle.vacunas),
                  ),
                  ...
                  _InfoCard(
                    ...
                    child: detalle.alergias.isEmpty
                        ? _EmptyState(message: 'No tiene alergias registradas')
                        : _AllergyList(alergias: detalle.alergias),
                  ),
                  ...
                  _InfoCard(
                    ...
                    child: detalle.proximasCitas.isEmpty
                        ? _EmptyState(message: 'No tiene próximas citas')
                        : _AppointmentList(citas: detalle.proximasCitas, isUpcoming: true),
                  ),
                  ...
                  _InfoCard(
                    ...
                    child: detalle.citasAnteriores.isEmpty
                        ? _EmptyState(message: 'No tiene citas anteriores')
                        : _AppointmentList(citas: detalle.citasAnteriores, isUpcoming: false),
                  ),
                  ...
                  _InfoCard(
                    ...
                    child: detalle.historialMedico.isEmpty
                        ? _EmptyState(message: 'No tiene historial médico')
                        : _MedicalHistoryList(historiales: detalle.historialMedico),
                  ),
...
// Widget para la sección de información básica del paciente
class _PatientInfoSection extends StatelessWidget {
  final dynamic detalle;
  const _PatientInfoSection({required this.detalle});
          ...
          child: mascota.fotoUrl != null && mascota.fotoUrl!.isNotEmpty
              ? Image.network(
                  ...
                  errorBuilder: (context, error, stackTrace) => Container(
                    ...
                    child: Icon(Icons.pets, color: AppColors.primary, size: 48),
                  ),
                )
              : Container(
                  width: 96,
...
// Widget para información del dueño
class _OwnerInfo extends StatelessWidget {
  final dynamic detalle;
  const _OwnerInfo({required this.detalle});
    ...
    return Column(
      ...
      children: [
        ...
        _InfoRow(
          icon: Icons.phone,
          label: 'Teléfono',
          value: dueno.telefono ?? 'No registrado',
        ),
        const SizedBox(height: 8),
        _InfoRow(
          icon: Icons.location_on,
          label: 'Dirección',
          value: dueno.direccion ?? 'No registrada',
        ),
        const SizedBox(height: 8),
        _InfoRow(
          icon: Icons.email,
...
// Tarjeta contenedora para secciones
...
class _AllergyList extends StatelessWidget {
  ...
  const _AllergyList({required this.alergias});
...
class _VaccineList extends StatelessWidget {
  final List<dynamic> vacunas;
  const _VaccineList({required this.vacunas});
...
class _AppointmentList extends StatelessWidget {
  ...
  const _AppointmentList({required this.citas, required this.isUpcoming});
          ...
          .map(
            (cita) => Padding(
              padding: EdgeInsets.only(
                bottom: citas.last == cita ? 0 : 12.0,
              ),
              child: _AppointmentItem(cita: cita, isUpcoming: isUpcoming),
            ),
          )
          .toList(),
    );
...
class _AppointmentItem extends StatelessWidget {
  final dynamic cita;
  final bool isUpcoming;
  const _AppointmentItem({required this.cita, required this.isUpcoming});
  ...
  Widget build(BuildContext context) {
    final textTheme = Theme.of(context).textTheme;
    final dateColor = isUpcoming ? AppColors.primary : AppColors.black60;
...
class _MedicalHistoryList extends StatelessWidget {
  final List<dynamic> historiales;
  const _MedicalHistoryList({required this.historiales});
            ...
            (historial) => Padding(
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      _formatFecha(historial.fechaConsulta),
                      style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                        fontWeight: FontWeight.w600,
                        color: AppColors.primary,
                      ),
                    ...
                    if (historial.notasAdicionales != null && historial.notasAdicionales!.isNotEmpty) ...[
                      const SizedBox(height: 8),
                      Text(
                        'Notas adicionales:',
                        style: Theme.of(context).textTheme.bodySmall?.copyWith(
    ...
    return ElevatedButton.icon(
      onPressed: () {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Función de descarga de PDF próximamente')),
        );
      },
